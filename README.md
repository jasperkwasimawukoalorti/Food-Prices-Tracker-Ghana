...
Bank CPI data into one region×month mart to explain — and forecast — maize price
swings in Ghana, with a live Streamlit dashboard.
...
3. **Analyse** — Maize real prices in Kumasi plotted against temperature and precipitation.
4. **Predict** — a lagged-price + weather model forecasts next-month maize price and is compared against a naive "previous month" baseline.
...
| | MSE |
|---|---|
| Model (lagged price + weather) | **311.66** |
| Baseline (previous month's price) | 408.75 |
